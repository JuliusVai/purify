...
function multiply(numbers, multiplier) {
  ...
  for (var i = 0; i < numberClone.length; i++) {
    numberClone[i] = numberClone[i] * multiplier;
  }
...
function concatNames(names) {
  ...
  for (var i = 0; i < names.length; i++) {
    newArr.push(`${names[i].firstName} ${names[i].lastName}`);
  }
  return newArr;
}
...
function numbersToStrings(things) {
  let thingsClone = [...things];
  for (var i = 0; i < thingsClone.length; i++) {
    thingsClone[i] =
      typeof thingsClone[i] === "number" ? thingsClone[i] + "" : thingsClone[i];
  }
...
function sortByLength(strings) {
  let stringsClone = [...strings];
  stringsClone.sort(function(a, b) {
...
function lastTwo(numbers) {
  let cloneNumber = [...numbers];

  const splicedNumbers = cloneNumber.splice(cloneNumber.length - 2, 2);
